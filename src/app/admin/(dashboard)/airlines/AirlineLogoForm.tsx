...
import { removeAirlineLogoAction, uploadAirlineLogoAction } from "@/app/admin/actions";

export function AirlineLogoForm({ airlineId, hasLogo }: { airlineId: string; hasLogo: boolean }) {
  ...
  function onUpload() {
    ...
    startTransition(async () => {
      try {
        await uploadAirlineLogoAction(airlineId, fd);
        setFileName(null);
  ...
  function onRemove() {
    ...
    startTransition(async () => {
      try {
        await removeAirlineLogoAction(airlineId);
        router.refresh();
      } catch (e) {
    ...
    <div className="mt-3">
      <div className="flex flex-wrap items-center gap-2">
        <label className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-outline-variant px-3 py-1.5 text-xs font-medium text-on-surface transition-colors hover:border-secondary">
          ...
          {fileName ? "Change file" : "Choose logo"}
          <input
            ref={inputRef}
            type="file"
            accept="image/*"
      ...
      <p className="mt-1.5 text-[0.65rem] text-on-surface-variant/70">
        Saved as WebP automatically.
      </p>
    </div>
  );
}
